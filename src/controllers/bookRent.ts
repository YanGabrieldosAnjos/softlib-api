...
export class BookRentController {
  async rentBook(userId: string, isbn: string): Promise<string> {
    try {
      ...
      const rentedBooks = await bookRentModel
        .find({ user: user?._id })
        .populate("book", "isbn title quantity");
  ...
  async devolutionBook(userId: string, isbn: string): Promise<string> {
    try {
      ...
      const bookToDevolve = rentedBooks.find(
        ({ book }) => book.isbn === bookToRent.isbn
      );
      ...
      await bookRentModel.updateOne(
        { _id: bookToDevolve._id },
        { isDeleted: true }
      );
