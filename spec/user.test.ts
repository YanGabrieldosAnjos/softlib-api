...
describe("user", () => {
    ...
    test("create", async ()=> {
        ...
        const userData = { username: faker.random.word(),  password: faker.random.word(), name: faker.random.word()};
    ...
    test("login", async () => {
        ...
        const token = await userRest.loginUser(userData.username, userData.password);
